...
import { getOneProjectFailure, getOneProjectSuccess } from "@/slice/project.slice";
...
const ShowVideosProject = () => {
  ...
  const getVideo = async (projectId, videoId) => {
    dispatch(getOneVideoStart());
    try {
      const { data } = await videoService.getOneVideoProject(projectId, videoId);
  ...
  return (
    <>
      <Helmet>
        <title>{`${project?.title} | ${video?.title}`}</title>
        <meta name="description" content={`Onlayn o'quv platformasi - ${video?.title}`} />
      </Helmet>
      <div className={`${mode ? "bg-white text-gray-700" : "bg-slate text-light"} scroll-smooth`}>
        <div className="w-full h-[60px] flex items-center justify-between px-3">
          <div className="w-full flex items-center justify-between gap-1 group">
            <Navbar />
          </div>
        </div>
        ...
        <div className={` flex items-start justify-start h-[calc(100vh-60px)] border-t border-gray-700 xs:flex-row flex-col gap-10 xs:gap-0`}>
          <div className={`md:w-[70%] xs:w-[60%] w-full h-[40%] xs:h-[70%] md:h-[80%] mb-3 ${mode ? "bg-light" : "bg-gray-800"}`}>
            {isLoading ? (
          ...
          <div
            className={`md:w-[30%] xs:w-[40%] w-full h-full overflow-y-scroll xs:border-none xs:pt-0pt-3 border-t ${
                  ...
                  {project?.videos?.length > 0 ? (
                    project?.videos?.map((video, vIndex) => (
                      <div
                        ...
                        className={`cursor-pointer p-2 ${playId === video._id ? "bg-blue-500 text-white" : "hover:bg-blue-500 hover:text-white"} ${
                          mode ? "text-gray-800" : "text-white"
                        }`}
                      >
        ...
        {commentModal && <AddComment refModel={"Project"} refId={project?.id} setCommentModal={setCommentModal} />}
      </div>
    </>
  );
};

export default ShowVideosProject;
